...
** The H4Reco dumper for BTL TB Sep 2025
If necessary, check the `outNameSuffix` and `path2data` paths in the `cfg/MTD_H8_Sep2025/MTD_H8_202509.cfg`
Run following command, changing `<start>`, `<end>`, and `<run_number>` accordingly to the spills and run number you want to process:
```
cd H4Analysis
for spill in {<start>...<end>}; do /afs/cern.ch/user/m/mtd/H4Analysis/bin/H4Reco /afs/cern.ch/user/m/mtd/H4Analysis/cfg/MTD_H8_Sep2025/MTD_H8_202509.cfg <run_number> ${spill}; done
```
** The H4Reco dumper
   - The main executable.
